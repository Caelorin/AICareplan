...
export default function CarePlanDetailPage() {
  ...
  // 按类别分组照护项目
  const groupedItems = CARE_CATEGORIES.map((category) => ({
    ...category,
        ...
        <Tabs defaultValue="plan" className="space-y-6">
                      ...
                      >
                        ...
                        <AccordionContent className="space-y-3 pb-4">
                          ...
                          <div className="knowledge-box knowledge-box--info">
                            <div className="mb-1.5 flex items-center gap-2 text-sm font-semibold" style={{ color: 'var(--edu-info)' }}>
                          ...
                          <div className="knowledge-box knowledge-box--success">
                            <div className="mb-2 flex items-center gap-2 text-sm font-semibold" style={{ color: 'var(--edu-success)' }}>
                              <CheckCircle2 className="h-4 w-4" />
                              具体措施
                            </div>
                            <ul className="space-y-1.5 text-sm">
                              {item.measures.map((measure, mIndex) => (
                                <li key={mIndex} className="flex items-start gap-2.5 leading-relaxed">
                                  <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: 'var(--edu-success)' }} />
                                  <span>{measure}</span>
                                </li>
                              ))}
                            </ul>
                          ...
                          <div className="knowledge-box knowledge-box--tip">
                            <div className="mb-1.5 flex items-center gap-2 text-sm font-semibold" style={{ color: 'var(--edu-tip)' }}>
                          ...
                          <div className="knowledge-box knowledge-box--warn">
                            <div className="mb-1.5 flex items-center gap-2 text-sm font-semibold" style={{ color: 'var(--edu-warn)' }}>
              ...
              <CardContent>
                <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                  {[
                    ...
                    { label: "性别", value: plan.elderInfo.gender === "male" ? "男" : "女" },
                    { label: "身高", value: `${plan.elderInfo.height} cm` },
                    { label: "体重", value: `${plan.elderInfo.weight} kg` },
                    {
                      label: "BMI",
                      value: `${bmi.toFixed(1)}`,
                      extra: bmi < 18.5 ? "偏瘦" : bmi < 24 ? "正常" : bmi < 28 ? "超重" : "肥胖",
                    },
                  ].map((item) => (
                ...
                <div className="grid gap-5 lg:grid-cols-2">
                  {[
                    { label: "主要诊断", value: plan.elderInfo.diagnosis },
                    { label: "既往病史", value: plan.elderInfo.medicalHistory },
                    { label: "当前用药", value: plan.elderInfo.medications },
                    { label: "过敏史", value: plan.elderInfo.allergies },
                    { label: "主要照护者", value: plan.elderInfo.primaryCaregiver || "未指定" },
                    {
                      label: "居住环境",
                      value:
                        plan.elderInfo.livingEnvironment === "alone"
                          ? "独居"
                          : plan.elderInfo.livingEnvironment === "with_family"
                            ? "与家人同住"
                            : "养老机构",
                    },
                  ].map((item) => (
          ...
          <TabsContent value="assessment" className="space-y-5">
            {ASSESSMENT_CATEGORIES.map((category) => {
              ...
              const categoryQuestions = Object.entries(plan.assessmentResult)
                .map(([key, value]) => getAssessmentDisplay(key, value))
                .filter((item) => item && item.category === category.id);
                      ...
                      {categoryQuestions.map(
                        (item, index) =>
                          item && (
                            ...
                            >
                              <p className="mb-1.5 text-xs font-medium text-muted-foreground leading-relaxed">
                                {item.question}
                              </p>
